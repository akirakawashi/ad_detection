...
Визуализация должна строиться только по финальному слою отображения:

```text
business_visible = 1
manual_ignore не отображается
render gap-fill применяется только для видео-оверлея
```

Нельзя строить красивые карточки по всем raw detections. Карточки должны соответствовать только тем bbox, которые реально показаны на annotated video.

Для каждого отображаемого объекта рисуется bbox и короткий label. Для основных объектов в кадре дополнительно рисуется компактная карточка с метриками:

```text
Class: OTHER / MTS / MIRANDA / PLUS7
Detection confidence: 91%
Brand confidence: 86%
Area in frame: 1.8%
Visibility score: 0.63
Overall score: 0.78
```

Поля `Status` и `Position` в карточке не показываем.

Если в кадре несколько объектов, карточки можно показывать для нескольких объектов, но с лимитом, например 4-5 карточек на кадр. Если объектов больше лимита, карточки получают top-N объектов по приоритету:

```text
целевые бренды выше other
больший area_ratio выше
больший overall_score выше
```

Остальные объекты остаются только с bbox и коротким label.

Карточки не обязаны быть справа. Их нужно размещать в свободных местах кадра: top-left, top-right, middle-left, middle-right, bottom-left, bottom-right или похожие слоты. При выборе слота нужно минимизировать пересечение с bbox объектов, уже размещенными карточками и краями кадра.

От карточки к bbox рисуется тонкая линия цвета бренда и точка-якорь. Положение карточки желательно стабилизировать по `object_id`, чтобы карточка не прыгала между сторонами кадра на каждом frame.

Важно: если объект `manual_review`, `unknown` или `not_classified`, нельзя рисовать карточку так, будто бренд подтвержден. Для бизнес-слоя такие объекты должны отображаться как `other`, если они вообще прошли `business_visible`.
