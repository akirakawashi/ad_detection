...
Docker image содержит только зависимости API и migrations. ML worker не
контейнеризируется: он запускается локально из проектной `.venv`, использует
локальные CUDA/GPU, модели и Python-зависимости.

Dev script запускает отсутствующие PostgreSQL/MinIO-контейнеры, ждёт их
готовности, применяет `alembic upgrade head`, запускает backend-контейнер,
если он ещё не работает, и затем запускает локальный worker.
