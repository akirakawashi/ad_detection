...
def _evaluate_one(detection: DetectionRecord, config: PipelineConfig) -> None:
    ...
    if (
        detection.crop_width < config.min_classify_width
        or detection.crop_height < config.min_classify_height
        or detection.area_ratio < config.min_classify_area_ratio
    ):
    ...
    if score >= config.crop_quality_pass_min:
        detection.crop_quality_status = "passed"
        detection.classification_input_status = "accepted"
    elif score >= config.crop_quality_borderline_min:
        detection.crop_quality_status = "borderline"
        detection.classification_input_status = "borderline"
    else:
        detection.crop_quality_status = "rejected"
        detection.classification_input_status = "rejected"
        detection.brand_status = "not_classified"
        detection.final_status = "not_classified"
...
def _blur_score(blur_var: float, config: PipelineConfig) -> float:
    if blur_var <= config.blur_borderline_variance:
        return 0.0
    if blur_var >= config.blur_pass_variance:
        return 1.0
    span = config.blur_pass_variance - config.blur_borderline_variance
    return (blur_var - config.blur_borderline_variance) / max(1e-9, span)
